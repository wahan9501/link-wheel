...
/* These variables are defined in other places */
function assert(v) {
  if (typeof v === "undefined") throw new Error();
}
assert(WHEEL_HTML_STRING);
assert(STORAGE_KEY_WHEEL_ITEMS);
...
function init(wheelItems) {
  if (!document.getElementById("wheel-container")) {
    document.body.insertAdjacentHTML("beforebegin", WHEEL_HTML_STRING);
